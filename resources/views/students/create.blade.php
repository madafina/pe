...
@section('content')
    <div class="card card-primary">
        <div class="card-header"><h3 class="card-title">Formulir Pendaftaran</h3></div>
        <form action="{{ route('students.store') }}" method="POST">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <h4>Data Siswa</h4><hr>
                        {{-- ... Input Nama, No HP, Alamat, Sekolah Asal (tidak berubah) ... --}}
                        ...
                        <div class="form-group">
                            ...
                            @error('parent_phone_number')<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>@enderror
                        </div>
                        <div class="form-group">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="registration_date_display" id="registration_date" class="form-control" value="{{ date('Y-m-d') }}" required>
                        </div>

                        {{-- PAKET BIMBEL (DINAMIS) --}}
                        <div class="form-group">
...
@section('js')
    <script>
        $(document).ready(function() {
            // Fungsi untuk mengambil data paket berdasarkan bulan
            async function fetchCoursePrices(monthName) {
                const coursePriceSelect = $('#course_price_id');
                coursePriceSelect.prop('disabled', true).html('<option>Loading...</option>');

                try {
                    const response = await fetch(`{{ route('api.course-prices') }}?month=${monthName}`);
                    ...
                    if (prices.length > 0) {
                        prices.forEach(price => {
                            ...
                            coursePriceSelect.append(
                                `<option value="${price.id}">${price.course.name} - ${price.enrollment_period} (Rp ${priceFormatted})</option>`
                            );
                        });
                    } else {
